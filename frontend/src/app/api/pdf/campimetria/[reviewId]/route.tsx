/**
 * Endpoint autenticado para descargar el PDF validado de Campimetría.
...
export async function GET(
  ...
  const filename = `Campimetria-${universalId}.pdf`
  ...
  if (review.validatedPdfUrl) {
    try {
      ...
      console.warn('[campimetria-pdf] No se pudo leer PDF persistido, regenerando:', fsErr)
    }
  }
  ...
  try {
    ...
    const data = await buildCampimetriaPdfDataAsync({
      reviewId: review.id,
      doctorStatus:
    ...
    const result = await generateCampimetriaValidatedPdf({
      reviewId: review.id,
      data,
    })
    ...
    if (result.url && !review.validatedPdfUrl) {
      try {
        ...
        console.warn('[campimetria-pdf] No se pudo persistir URL regenerada:', persistErr)
      }
    }
    ...
    console.error('[campimetria-pdf] Error generando PDF en línea:', err)
