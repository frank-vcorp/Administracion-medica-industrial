 ...
 *   - Lista de empresas a eliminar (scrollable si > 5)
 *   - Advertencia roja de irreversibilidad
...
interface Props {
  selectedNames: Array<{ id: string; name: string; rfc: string }>
  onClearSelection: () => void
}

export default function DeleteCompaniesButton({ selectedNames, onClearSelection }: Props) {
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-slate-900 text-white shadow-lg border-t border-slate-700">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <span className="font-bold text-sm">
            {selectedNames.length} {selectedNames.length === 1 ? 'empresa seleccionada' : 'empresas seleccionadas'}
          </span>
      ...
      {open && (
        ...
        >
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
            <h3 id="delete-modal-title" className="text-xl font-black text-slate-900 mb-2">
              Eliminar {selectedNames.length}{' '}
              {selectedNames.length === 1 ? 'empresa' : 'empresas'}
            </h3>
              ...
              <ul className="text-sm space-y-1">
                {selectedNames.map((s) => (
                  <li key={s.id} className="flex justify-between gap-2">
                    ...
                    <span className="font-mono text-slate-400 text-xs">{s.rfc || '—'}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="bg-red-50 border border-red-200 text-red-800 text-sm rounded-lg p-3 mb-4">
              <strong className="font-bold">⚠️ Atención:</strong> Esta acción es IRREVERSIBLE.
              Se eliminarán las empresas y se desvincularán sus workers, appointments, projects, etc.
              La historia clínica se preserva (los registros quedan con empresa=null).
            </div>
            ...
            <label className="block mb-4">
              ...
              <input
                ...
                placeholder="Ej. cliente dado de baja, solicitud del admin"
            ...
            <label className="flex items-start gap-2 mb-4 cursor-pointer">
              ...
              <span className="text-sm text-slate-700">
                Entiendo que esto es irreversible y no se puede deshacer.
              </span>
            </label>
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {isPending
                  ? 'Eliminando…'
                  : `Eliminar ${selectedNames.length} ${selectedNames.length === 1 ? 'empresa' : 'empresas'}`}
              </button>
            </div>
          </div>
        </div>
