...
def _make_db_row(provider="m3", api_key="sk-from-db-9f3a", enabled=True,
    ...
    FIX-20260810-02: el row debe parecerse al resultado del prisma client
    real. prisma-client-py 0.15 expone columnas Bytes como base64 str (no
    bytes). El resolver hace `base64.b64decode(row.X)` antes de descifrar;
    si guardamos bytes crudos aquí, el resolver fallaría con TypeError en
    producción.
    """
...
def test_resolve_db_corrupt_single_byte_in_ciphertext_decrypt_error(
    ...
    assert resolution.source == "env"
    assert resolution.warning == "decrypt_error"
    assert resolution.api_key == "env-dr7-fallback"
